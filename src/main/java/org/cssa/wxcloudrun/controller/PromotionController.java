...
public class PromotionController {
    ...
    @RequestMapping(value = {"/getOngoingMainPromotions"}, method = {RequestMethod.GET})
    @Operation(summary = "获取正在进行中的推广活动", description = "根据类型获取当前正在进行中的推广活动。如果类型为 null，则返回所有类型的活动。")
    public Response<List<Promotion>> getOngoingMainPromotions(@RequestParam(required = false) @Parameter(description = "推广活动的类型，如0为广告，1为活动。可为空。") Integer type) {
